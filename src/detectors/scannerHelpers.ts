 ...
 */
export function isFalsePositive(line: string, filePath: string): boolean {
 ...
 */
export function isTestKey(line: string, filePath: string, matchValue: string): boolean {
  const lowerLine = line.toLowerCase();
  const lowerPath = filePath.toLowerCase();
 ...
 */
export function isCommentLine(line: string, platforms: Platform[]): boolean {
 ...
 */
export function isLowConfidenceMatch(matchValue: string, patternName: string, varName: string = ''): boolean {
  if (varName) {
 ...
 */
export function collectXmlCommentRanges(content: string): [number, number][] {
 ...
 */
export function sanitizeEnvName(raw: string): string {
  return raw
    .toUpperCase()
    .replace(/[^A-Z0-9_]+/g, '_')
    .replace(/_+/g, '_')
...
export function extractMatchDetails(
  ...
  content: string,
  ext: string
): { matchValue: string; varName: string } {
