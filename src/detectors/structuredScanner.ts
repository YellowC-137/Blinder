...
type StructuredFileType = ClassifyKeyInput['fileType'];

/**
 * 구조화 파일 스캔. 결과는 results 배열에 push.
 *
 * @param filePath  절대 경로
 * @param repoPath  리포지토리 루트 (relPath 계산용)
 * @param content   파일 전체 내용
 * @param results   출력 누적 배열
 * @param usedEnvNames  envVarName 충돌 회피용 (base → value)
 */
export function scanStructuredFile(
  filePath: string,
  repoPath: string,
  content: string,
  results: CodeSecretMatch[],
  ...
  let fileType: StructuredFileType | null = null;
  ...
  try {
    ...
    logger.debug(`Parser failed for ${relPath}: ${(err as Error).message}`);
    return;
  }
