 ...
 */
function findPlatformForExtension(platforms: Platform[], ext: string): Platform {
 ...
 * expression.
 */
function findEnclosingStringLiteral(lineContent: string, matchStart: number, matchEnd: number): StringLiteralBounds | null {
  let openIdx = -1;
  let openChar = '';
  let inString = false;
  let curChar = '';
 ...
 */
export async function applyAutoFixes(repoPath: string, selectedSecrets: ScanResult[], options: ProtectionOptions = {}): Promise<Migration[]> {
  const platforms: Platform[] = options.platforms || [];
  const fileGroups = selectedSecrets.reduce<Record<string, ScanResult[]>>((acc, res) => {
  ...
  for (const [relPath, fileSecrets] of Object.entries(fileGroups)) {
    ...
    if (matchingPlatform?.preFix) {
      try {
        ...
        throw new Error(t('service_prefix_failed', { name: matchingPlatform.id || matchingPlatform.name, file: relPath, msg: (err as Error).message }));
      }
    }
    ...
    try {
      ...
      logger.error(t('service_read_failed', { file: relPath, msg: (err as Error).message }));
      continue;
    }
    ...
    if (fileModified && !options.dryRun) {
      try {
        ...
        logger.error(t('service_write_failed', { file: relPath, msg: (err as Error).message }));
      }
    }
      ...
      for (const s of fileSecrets) {
        ...
        try {
          await matchingPlatform.postFix({ 
            ...
            options, 
            ext, 
            envVarName: cs.envVarName 
          });
        } catch (err) {
          logger.warn(t('service_postfix_failed', { file: relPath, env: cs.envVarName, msg: (err as Error).message }));
        }
      }
    }
  }
...
interface EnvResult {
  selected: Array<ScanResult & { envVarName: string; secretValue: string }>;
  envContent: string;
