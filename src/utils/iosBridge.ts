...
const BLINDER_RUBY_HOOK: string = `
# --- Blinder Hook Start ---
 ...
 */
async function injectToPodfile(repoPath: string): Promise<boolean> {
  const podfilePaths: string[] = await (await import('fast-glob')).default('**/Podfile', {
    cwd: repoPath,
  ...
  const podfilePath: string = podfilePaths[0];
  logger.info(t('ios_bridge_applying', { path: path.relative(repoPath, podfilePath) }));

  let content: string = fs.readFileSync(podfilePath, 'utf8');
  ...
  const hasHookCall: boolean = /^\s*blinder_post_install\(installer\)/m.test(content);
...
export async function setupIosBridge(repoPath: string): Promise<void> {
  const scriptPath: string = path.join(repoPath, 'blinder-ios-setup.sh');
  ...
  const autoInjected: boolean = await injectToPodfile(repoPath);

  if (!autoInjected) {
    try {
      execFileSync('sh', [scriptPath], { stdio: 'inherit', cwd: repoPath });
    } catch (err: unknown) {
      logger.error(t('ios_bridge_setup_failed', { msg: (err as Error).message }));
    }
