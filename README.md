...
| 🔁 **AI 수정안을 원본에 머지** — 토큰을 다시 시크릿으로 돌리는 작업이 수동/위험 | `restore`가 `.blinder_map.json` 기반으로 자동 복원 |
...
❌ 절대 안 됩니다. `__BLINDER_VAR__` 토큰 때문에 컴파일 에러 발생. 사본은 **읽기 전용**.
...
**Q. `.blinder_protect.json` / `.blinder_map.json`을 git에 커밋해야 하나요?**
❌ 두 파일 모두 `.gitignore`에 자동 추가됨. 로컬에서도 절대 삭제 금지.
