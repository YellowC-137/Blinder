...
- **🔌 플러그인 기반 확장**: IPlatform 인터페이스를 구현하는 것만으로 새로운 언어/프레임워크를 지원할 수 있습니다. 코어 엔진 수정 없이 `src/platforms/`에 파일 하나만 추가하면 전체 파이프라인에 통합됩니다.
...
대화형 프롬프트에 답하면 플러그인 파일 생성 + 레지스트리 등록이 **자동으로** 완료됩니다. 자세한 내용은 [CONTRIBUTING.md](./CONTRIBUTING.md)를 참고하세요.
