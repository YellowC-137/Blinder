...
- **💬 주석 시크릿 옵트인 스캔**: 테스트/상용 값을 주석으로 토글하는 설정 파일을 위해, `blind`/`mask` 실행 시 "주석 안 시크릿도 스캔할까요?" 프롬프트 제공. 발견되면 별도 섹션으로 보고하되 auto-fix는 건너뛰고 수동 삭제를 권고. (`scan --scan-comments`로 직접 사용 가능)
- **🚫 인터랙티브 디렉토리 제외**: `blind`/`mask` 실행 시 사용자에게 추가로 제외할 폴더(외부 SDK, 빌드 산출물 등)를 콤마 구분 glob 패턴으로 입력받아 즉시 적용.
- **🌐 다국어 지원 (한국어 / English)**: 최초 실행 시 언어 선택 프롬프트(`1. English` / `2. 한국어`) 자동 표시. 이후 모든 출력이 선택 언어로 렌더링. `blinder set_language ko|en`으로 변경 가능.
...
설치 후 처음 어떤 명령(예: `blinder scan`)을 실행하면 다음과 같은 프롬프트가 자동으로 표시됩니다.
...
선택한 언어는 `~/.blinder/config.json`에 저장되며, 이후 모든 명령의 출력(설명, 프롬프트, 로그, 에러 메시지)이 해당 언어로 렌더링됩니다. 비-TTY 환경(CI / pipe / `--yes`)에서는 자동으로 `en` 기본값이 저장되어 파이프라인이 차단되지 않습니다.

언어 변경:

```bash
blinder set_language ko    # 한국어로 전환
blinder set_language en    # English로 전환
```
...
**인터랙티브 프롬프트** (순서대로 노출):
1. **주석 스캔 여부**: "주석 처리된 코드 안의 시크릿도 스캔할까요?" — 테스트/상용 값을 주석으로 토글하는 설정 파일에 유용. 기본 `No`.
...
CLI 출력 언어를 한국어 / English로 전환. 설정은 `~/.blinder/config.json`에 저장되며 즉시 적용. 최초 실행 시 자동 프롬프트로 설정되므로 보통 이 명령은 사후 변경용.
...
<details>
<summary><strong>Q. 외부 SDK 폴더도 스캔되나요?</strong></summary>
...
<details>
<summary><strong>Q. 주석 처리된 시크릿(예: 테스트/상용 토글)도 탐지되나요?</strong></summary>

기본은 **탐지하지 않습니다** (false-positive 방지). `blind` / `mask` 실행 시 "주석 안 시크릿도 스캔할까요?" 프롬프트에서 옵트인하거나, `blinder scan --scan-comments` 플래그로 직접 활성화할 수 있습니다.

옵트인하면 주석 라인의 시크릿이 별도 `💬 Commented-out Secrets` 섹션에 보고되지만 **auto-fix는 건너뜁니다** — 이미 dead code이므로 env 치환은 무의미하기 때문입니다. 대신 사용자에게 해당 라인을 직접 삭제하도록 권고합니다.

⚠️ 주의: 주석으로 토글한 prod URL/키도 git에 그대로 올라가면 동일한 위험을 가집니다 — 발견되면 회전(rotate) + 삭제 권장.
</details>
...
<details>
<summary><strong>Q. CLI 출력 언어를 바꾸고 싶어요.</strong></summary>

설치 후 처음 명령을 실행하면 `1. English` / `2. 한국어` 선택 프롬프트가 자동 표시됩니다. 사후에는 `blinder set_language ko` 또는 `blinder set_language en`으로 변경할 수 있습니다. 설정 파일은 `~/.blinder/config.json`에 저장됩니다.

CI 등 비-TTY 환경에서는 자동으로 `en`이 저장되어 파이프라인이 차단되지 않습니다.
</details>
...
> [!WARNING]
> **벤더 라이브러리 빌드 영향**: 사내 보안 및 핵심 라이브러리는 키 길이 검증 등 자체 제약이 있을 수 있음. `.blinderSettings`의 `ignorePaths`로 제외 후 적용 권장.
