...
키값을 `.env` 등으로 분리하여 **"코드는 계속 실행 가능하도록"** 유지하면서 안전하게 공유하는 것이 목적입니다. 프로젝트 내의 시크릿을 탐지하고 `.env`로 마이그레이션하여 프로젝트 보안 기초를 다집니다. `scan` + `protect` + `gitignore` 과정을 한 번에 수행합니다.
...
프로그램의 실행 가능 여부와 상관없이, AI가 **"순수하게 비즈니스 로직과 코드의 흐름만"** 읽을 수 있도록 민감한 값(상수, 문자열 등)을 완전히 덮어씌우거나 제외하는 것이 목적입니다. AI 에이전트(Cursor 등)에게 프로젝트 전체의 맥락을 제공하면서도 시크릿은 유출되지 않도록 **마스킹된 프로젝트 복사본**을 생성합니다. 모든 시크릿은 `__BLINDER_VAR__` 태그로 치환되며, SSH 키 등 불필요한 보안 파일은 제외됩니다.

> [!NOTE]
> **왜 `.env` 파일 접근 권한만 차단하지 않고 굳이 `mask` 기능을 사용해야 할까요?**
> 1. **원본 유지 (비침투적)**: `blind`는 원본 코드를 수정하지만, `mask`는 원본을 유지한 채 안전한 **물리적 복사본**을 만듭니다. 원본 코드 변경을 원치 않을 때 유용합니다.
> 2. **외부 공유**: 로컬 AI 에이전트뿐만 아니라 ChatGPT 웹이나 타인에게 코드를 압축해서 전달할 때 시크릿이 물리적으로 제거된 폴더가 필요합니다.
> 3. **실행 로그 유출 방지**: 에이전트가 코드를 빌드/실행할 때, 터미널 로그나 에러 메시지에 메모리에 적재된 시크릿이 출력될 수 있습니다.
> 4. **Git 기록 보호**: 파일 접근을 막아도 이전 Git 커밋 내역에 남아있는 하드코딩된 시크릿을 에이전트가 읽어낼 위험이 있습니다.
