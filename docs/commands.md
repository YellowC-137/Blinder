...
> AI에 코드를 노출할 때 시크릿만 가리는 두 가지 방식: **사본 방식**(`mask`/`restore` — 복붙 공유용, 실행 불가)과 **훅 방식**(`hook` — Claude Code용, 사본 없이 실시간 마스킹).

### B-1. `blinder mask` — 복붙 공유용 사본 생성 (실행 불가)

프로젝트 전체 또는 지정 하위 디렉토리를 복사하여 `maskedProject_<projectName>/` 생성. 모든 시크릿은 `__BLINDER_<환경변수명>__` 토큰으로 치환되며, 다음 항목은 사본에서 **완전 제외**:
...
시크릿 매핑은 **사본 밖** — `<프로젝트 루트>/.blinder_maps/<사본폴더명>.json` 에 저장되어 `restore` 시 사용됩니다. 사본 안에는 시크릿이 일절 남지 않으므로 사본 폴더를 통째로 AI에 공유해도 안전합니다.

> [!CAUTION]
> ## 🚨 `mask` 결과물은 **실행 불가**합니다 🚨
> - ❌ **빌드/실행 절대 시도 금지** — 모든 시크릿 값이 `__BLINDER_*__` 토큰으로 치환되어 컴파일 에러 또는 런타임 NPE 발생
...
AI가 마스킹된 사본에서 작업한 **모든 코드 변경 + 신규 파일**을 원본 프로젝트로 안전하게 가져옴. `__BLINDER_*__` 토큰은 자동으로 실제 시크릿 값으로 복원. import 누락 등의 정합성 자동 보정 포함. 복원 후 정리(cleanup)를 선택하면 사본 폴더와 해당 매핑 파일이 함께 삭제됩니다.

매핑 탐색 순서: ① `<프로젝트 루트>/.blinder_maps/<사본폴더명>.json` → ② (구버전 호환) 사본 안의 `.blinder_map.json`.

> [!WARNING]
> `.blinder_maps/` 의 매핑 파일이 변조/삭제되면 시크릿 복원 불가. `restore` 가 끝나기 전까지 삭제 금지.
...
감지된 플랫폼별 템플릿(.env, build/, *.jks, ...) + Blinder 생성 파일을 `.gitignore`에 추가. 각 섹션은 `# --- BLINDER <ID> ---` / `# --- BLINDER <ID> END ---` 마커로 감싸여, `rollback` 시 사용자가 직접 추가한 줄은 건드리지 않고 Blinder 블록만 정확히 제거됩니다.

### C-3. `blinder add_platform` — 신규 플랫폼 스캐폴더

대화형으로 플러그인 파일 1개(`.ts`) + `index.ts` 등록을 자동 생성. [아키텍처 가이드](./architecture.md) 참고.
